<?php
require_once "inc/auth.php";
require_once "../config/database.php";
require_once "../config/helpers.php";

checkAdminLogin();

$db = new Database();
$conn = $db->connect();

$stmt = $conn->query("
    SELECT p.*, (SELECT COUNT(*) FROM subscriptions s WHERE s.payment_method_id = p.id) AS total_sub
    FROM payment_methods p
    ORDER BY p.method_name
");
$methods = $stmt->fetchAll();

$pageTitle = "Metode Pembayaran";
include "templates/header.php";
include "templates/sidebar.php";
include "templates/navbar.php";
?>

<div class="admin-content">

    <div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-1">
        <div>
            <div class="admin-page-title">Metode Pembayaran</div>
            <p class="admin-page-sub mb-0">Kelola daftar metode pembayaran yang tersedia untuk seluruh pengguna.</p>
        </div>
        <button class="admin-btn-primary" data-bs-toggle="modal" data-bs-target="#addModal">
            <i class="fa-solid fa-plus me-2"></i>Tambah Metode
        </button>
    </div>

    <?php renderFlash(); ?>

    <div class="admin-card">
        <?php if (count($methods) > 0): ?>
        <div class="table-responsive">
        <table class="admin-table">
            <thead><tr><th>Metode</th><th>Provider</th><th>Total Subscription</th><th>Aksi</th></tr></thead>
            <tbody>
            <?php foreach ($methods as $m): ?>
                <tr>
                    <td><strong><?= htmlspecialchars($m['method_name']) ?></strong></td>
                    <td><?= htmlspecialchars($m['provider'] ?: '-') ?></td>
                    <td><?= $m['total_sub'] ?></td>
                    <td>
                        <div class="d-flex gap-2">
                            <button class="btn btn-sm admin-btn-outline edit-method"
                                data-bs-toggle="modal" data-bs-target="#editModal"
                                data-id="<?= $m['id'] ?>" data-name="<?= htmlspecialchars($m['method_name']) ?>"
                                data-provider="<?= htmlspecialchars($m['provider']) ?>">
                                <i class="fa-solid fa-pen"></i>
                            </button>
                            <form action="payment-methods-process.php" method="POST" class="confirm-delete"
                                data-confirm="Hapus metode pembayaran <?= htmlspecialchars($m['method_name']) ?>?">
                                <input type="hidden" name="action" value="delete">
                                <input type="hidden" name="id" value="<?= $m['id'] ?>">
                                <button class="btn btn-sm btn-outline-danger"><i class="fa-solid fa-trash"></i></button>
                            </form>
                        </div>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
        </div>
        <?php else: ?>
            <div class="admin-empty"><i class="fa-solid fa-money-check-dollar"></i>Belum ada metode pembayaran.</div>
        <?php endif; ?>
    </div>

</div>

<!-- Add Modal -->
<div class="modal fade" id="addModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form action="payment-methods-process.php" method="POST">
        <input type="hidden" name="action" value="add">
        <div class="modal-header border-0 pb-0">
          <h5 class="modal-title fw-bold">Tambah Metode Pembayaran</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
            <div class="mb-3">
                <label>Nama Metode</label>
                <input type="text" name="method_name" class="form-control" placeholder="Bank Transfer / E-Wallet / Credit Card" required>
            </div>
            <div class="mb-2">
                <label>Provider</label>
                <input type="text" name="provider" class="form-control" placeholder="BCA, DANA, Visa, dll">
            </div>
        </div>
        <div class="modal-footer border-0">
          <button type="button" class="btn admin-btn-outline" data-bs-dismiss="modal">Batal</button>
          <button type="submit" class="admin-btn-primary">Simpan</button>
        </div>
      </form>
    </div>
  </div>
</div>

<!-- Edit Modal -->
<div class="modal fade" id="editModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form action="payment-methods-process.php" method="POST">
        <input type="hidden" name="action" value="edit">
        <input type="hidden" name="id" id="edit_id">
        <div class="modal-header border-0 pb-0">
          <h5 class="modal-title fw-bold">Edit Metode Pembayaran</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
            <div class="mb-3">
                <label>Nama Metode</label>
                <input type="text" name="method_name" id="edit_name" class="form-control" required>
            </div>
            <div class="mb-2">
                <label>Provider</label>
                <input type="text" name="provider" id="edit_provider" class="form-control">
            </div>
        </div>
        <div class="modal-footer border-0">
          <button type="button" class="btn admin-btn-outline" data-bs-dismiss="modal">Batal</button>
          <button type="submit" class="admin-btn-primary">Update</button>
        </div>
      </form>
    </div>
  </div>
</div>

<script>
document.querySelectorAll('.edit-method').forEach(el => {
    el.addEventListener('click', function () {
        document.getElementById('edit_id').value = this.dataset.id;
        document.getElementById('edit_name').value = this.dataset.name;
        document.getElementById('edit_provider').value = this.dataset.provider;
    });
});
</script>

<?php include "templates/footer.php"; ?>
