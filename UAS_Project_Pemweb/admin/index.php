...
$totalRevenue = $conn->query("SELECT COALESCE(SUM(amount),0) FROM subscriptions WHERE status='Active'")->fetchColumn();
$newUsersThisMonth = $conn->query("SELECT COUNT(*) FROM users WHERE EXTRACT(MONTH FROM created_at)=EXTRACT(MONTH FROM CURRENT_DATE) AND EXTRACT(YEAR FROM created_at)=EXTRACT(YEAR FROM CURRENT_DATE)")->fetchColumn();
