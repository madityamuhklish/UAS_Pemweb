...
$stmt = $conn->prepare("
    SELECT COALESCE(c.category_name,'Lainnya') AS category_name, SUM(s.amount) AS total
...
$stmt = $conn->prepare("
    SELECT TO_CHAR(ph.payment_date, 'YYYY-MM') AS ym, SUM(ph.amount) AS total
    FROM payment_history ph
    JOIN subscriptions s ON s.id = ph.subscription_id
    WHERE s.user_id = ? AND ph.payment_date >= (CURRENT_DATE - INTERVAL '6 months')
    GROUP BY ym
    ORDER BY ym ASC
");
$stmt->execute([$userId]);
...
/* Summary */
$stmt = $conn->prepare("SELECT COALESCE(SUM(amount),0) FROM subscriptions WHERE user_id=? AND status='Active'");
$stmt->execute([$userId]);
...
$stmt = $conn->prepare("SELECT COALESCE(AVG(amount),0) FROM subscriptions WHERE user_id=? AND status='Active'");
$stmt->execute([$userId]);
...
$stmt = $conn->prepare("
    SELECT service_name, amount, next_payment, DATEDIFF(next_payment, CURRENT_DATE) AS days_left
    FROM subscriptions
    WHERE user_id = ? AND status = 'Active'
      AND next_payment BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL '7 days')
