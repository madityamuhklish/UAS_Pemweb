...
/**
 * Kalau berjalan di Vercel (atau hosting lain yang di-set env DB_HOST),
 * pakai session handler berbasis database supaya login tetap konsisten
 * antar serverless function. Kalau di lokal (XAMPP), pakai session file
 * bawaan PHP seperti biasa (tidak ada perubahan perilaku).
 */
if (getenv('DB_HOST') && session_status() === PHP_SESSION_NONE) {
    require_once __DIR__ . '/database.php';
    require_once __DIR__ . '/db_session_handler.php';

    $sessionDb = new Database();
    $sessionConn = $sessionDb->connect();

    session_set_save_handler(new DbSessionHandler($sessionConn), true);
}
